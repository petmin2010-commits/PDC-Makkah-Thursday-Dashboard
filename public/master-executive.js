...
function renderCharts(rows){
 ...
 const months={};rows.forEach(z=>{let d=null;try{d=typeof parseSheetDate==='function'?parseSheetDate(z.assignedDate):new Date(z.assignedDate)}catch{}if(!d||isNaN(d))return;const k=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0');if(!months[k])months[k]={count:0,value:0};months[k].count++;months[k].value+=n(z.value)});const keys=Object.keys(months).sort();draw('meMonthly','bar',keys,[{type:'bar',label:'قيمة الإسناد',data:keys.map(k=>months[k].value),backgroundColor:'rgba(37,99,235,.4)',yAxisID:'y'},{type:'line',label:'عدد أوامر العمل',data:keys.map(k=>months[k].count),borderColor:'#f59e0b',backgroundColor:'#f59e0b',tension:.25,yAxisID:'y1'}],{scales:{y:{beginAtZero:true,position:'right'},y1:{beginAtZero:true,position:'left',grid:{drawOnChartArea:false}}}});
}
