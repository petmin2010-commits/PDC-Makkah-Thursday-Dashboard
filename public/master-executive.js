...
function render(rows){
 ...
 <section class="me-charts">
   ...
   ${chart('meSectionCount','القسم — حسب العدد','SECTION / COUNT')}${chart('meSectionValue','القسم — حسب الإسناد بالريال','SECTION / ASSIGNMENT VALUE')}
...
function renderCharts(rows){
 ...
 const sectionGroups=group(rows,'section').filter(x=>x[0]&&x[0]!=='غير محدد').slice(0,8);
 const sections=sectionGroups.map(x=>x[0]),sectionCounts=sectionGroups.map(x=>x[1]),sectionValues=sections.map(k=>sum(rows.filter(z=>(t(z.section)||'غير محدد')===k),'value'));
 const sectionColors=['#0b0b8d','#4045b8','#6668c8','#8a8ce0','#acaef0','#d1d2fb','#7c3aed','#2563eb'];
 draw('meSectionCount','doughnut',sections,[{label:'عدد أوامر العمل',data:sectionCounts,backgroundColor:sectionColors,borderColor:'#fff',borderWidth:1}],{cutout:'60%',plugins:{legend:{position:'right',labels:{usePointStyle:true,font:{family:'Cairo',size:9}}},tooltip:{callbacks:{label:c=>{const v=Number(c.raw||0),tot=sectionCounts.reduce((a,b)=>a+b,0)||1;return c.label+': '+v.toLocaleString('ar-SA')+' ('+((v/tot)*100).toFixed(2)+'%)'}}}}});
 draw('meSectionValue','doughnut',sections,[{label:'قيمة الإسناد',data:sectionValues,backgroundColor:sectionColors,borderColor:'#fff',borderWidth:1}],{cutout:'60%',plugins:{legend:{position:'right',labels:{usePointStyle:true,font:{family:'Cairo',size:9}}},tooltip:{callbacks:{label:c=>{const v=Number(c.raw||0),tot=sectionValues.reduce((a,b)=>a+b,0)||1;return c.label+': '+v.toLocaleString('ar-SA',{maximumFractionDigits:2})+' ر.س ('+((v/tot)*100).toFixed(2)+'%)'}}}}});
 draw('meSection','bar',sections,['تم التنفيذ','لم يتم التنفيذ','موقوف/محول'].map((s,i)=>({label:s,data:sections.map(k=>rows.filter(z=>(t(z.section)||'غير محدد')===k&&status(z)===s).length),backgroundColor:['#16a34a','#f59e0b','#64748b'][i]})),{scales:{x:{stacked:true},y:{stacked:true}}});
